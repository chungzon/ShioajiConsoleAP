...
class RealtimeMonitorView(tk.Frame):
    ...
    def init_ui(self):
        ...
        self.tree.grid(row=1, column=0, columnspan=6, rowspan=2, sticky='nsew')
        vsb.grid(row=2, column=6, sticky='ns')
        hsb.grid(row=2, column=0, columnspan=6, rowspan=2, sticky='ew')
        
        self.grid_rowconfigure(1, weight=1)
        self.grid_columnconfigure(0, weight=1)
        
        self.radio_var = tk.StringVar(value="RealTime")  # 設置變數來儲存選擇的值
        
        # 添加 Radio 按鈕
        radio1 = ttk.Radiobutton(self, text="即時資料", variable=self.radio_var, value="RealTime")
        radio2 = ttk.Radiobutton(self, text="回測", variable=self.radio_var, value="History")
        
        radio1.grid(row=0, column=0, padx=10, pady=10, sticky='w')
        radio2.grid(row=0, column=1, padx=10, pady=10, sticky='w')
        
        ttk.Label(self, text="股票代碼:").grid(row=0, column=2, padx=10, pady=5, sticky='e')
        self.stock_id_entry = ttk.Entry(self)
        self.stock_id_entry.grid(row=0, column=3, padx=10, pady=5, sticky='e')

        ttk.Label(self, text="開始日期 (YYYY-MM-DD):").grid(row=0, column=4, padx=10, pady=5, sticky='e')
        ...
        self.start_date_ticks_entry.grid(row=0, column=5, padx=10, pady=5, sticky='w')

        ttk.Label(self, text="結束日期 (YYYY-MM-DD):").grid(row=0, column=6, padx=10, pady=5, sticky='e')
        ...
        self.end_date_ticks_entry.grid(row=0, column=7, padx=10, pady=5, sticky='w')
        ttk.Button(self, text="分析資料", command=self.print_message).grid(row=0, column=8, pady=20)
        
        # 使用 tkintertable 初始化下方表格，但暂时不填充数据
        self.table_frames = [
            self.create_tkintertable_frame("分均線", {}).grid(row=1, column=6, columnspan=3, sticky='nsew', padx=5, pady=5),
            self.create_tkintertable_frame("一分K均線", {}).grid(row=2, column=6, columnspan=3, sticky='nsew', padx=5, pady=5),
            self.create_tkintertable_frame("三分K均線", {}).grid(row=3, column=6, columnspan=3, sticky='nsew', padx=5, pady=5),
            self.create_tkintertable_frame("五分K均線", {}).grid(row=4, column=6, columnspan=3, sticky='nsew', padx=5, pady=5),
        ]

        # 第一個圖表
        self.fig1, self.ax1 = plt.subplots()
        self.canvas1 = FigureCanvasTkAgg(self.fig1, self)
        self.canvas1.get_tk_widget().grid(row=3, column=0, rowspan=2, columnspan=3, padx=10, pady=10, sticky='nsew')
        
        # 第二個圖表
        self.fig2, self.ax2 = plt.subplots()
        self.canvas2 = FigureCanvasTkAgg(self.fig2, self)
        self.canvas2.get_tk_widget().grid(row=3, column=3, rowspan=2, columnspan=3, padx=10, pady=10, sticky='nsew')
    ...
    def print_message(self):
        def process():
            ...
            if self.radio_var.get() == 'RealTime':
                for i in range(0,4):
