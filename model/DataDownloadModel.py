    ...
    def get_all_stocks_kbars(self, start_date, end_date):
        ...
        for stock_id in top_stocks:
            try:
                ...
                self.event_bus.publish(Event("log_message", f"下載股票 {stock_id}  {start_date} 到 {end_date} 的KBar資料"))
                self.logger.info(f"下載股票 {stock_id}  {start_date} 到 {end_date} 的KBar資料")
            except Exception as e:
                self.event_bus.publish(Event("log_message", f"下載股票 {stock_id}  {start_date} 到 {end_date} 的KBar資料失敗: {e}"))
